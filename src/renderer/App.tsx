...
function App() {
  ...
  return (
    <div
      className="flex flex-col items-center justify-center mb-8  text-blue-700  w-full
     py-4 px-8  font-black
     "
    >
      <h1
        className="block mt-5 mb-4  drop-shadow-2xl font-black text-2xl
      font-sans
      "
      >
        {' '}
        Buscar pela divergencias entre XML e consulta de faturamento ou sentença
        venda
      </h1>
          ...
          {jsonXmlList.length > 0 && (
            <button
             ...
             rounded-full mb-4 ml-8 cursor-pointer hover:bg-blue-200"
              onClick={() =>
                ClearCache(setJsonXmlList, setSystemList, setMissingNote)
              }
            >
              <MdReplay size={25} className="flex mr-1" />
              <div className="flex ml-1">Limpar Dados</div>
            </button>
          )}
        </div>
      </form>
      ...
      {jsonXmlList.length > 0 && (
        <>
          ...
          <label className="block mb-1">
            ...
            >
              <MdOutlinePostAdd size={25} className="mr-2" />
              Enviar consulta de faturamento ou sentença vendas para comparar
            </span>
            <input
                  ...
                  <tbody className="  w-full mt-3">
                    {missingNote.length !== 0
                      ? missingNote.map((item: IXmlItem, index: number) => {
                          return ListItens(item, index, style);
                        })
                      : ''}
                  </tbody>
                </table>
